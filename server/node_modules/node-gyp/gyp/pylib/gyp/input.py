...
def EvalCondition(condition, conditions_key, phase, variables, build_file):
  ...
  if len(condition) < 2:
    ...
    raise GypError((f'{conditions_key} {condition[0]}' +
...
def ProcessVariablesAndConditionsInList(the_list, phase, variables,
  ...
  while index < len(the_list):
    ...
    if type(item) is dict:
      ...
      raise TypeError(
          ((f'Unknown type {item.__class__.__name__}' + ' at index ') + index))
    index += 1
    ...
    for dependency_key in all_dependency_sections:
      ...
      for index in xrange(0, len(dependencies)):
        ...
        if dependency_key != 'dependencies' and \
          ...
          raise GypError(
              (((f'Found {dependency} in {dependency_key}' + ' of ') + target)
    ...
    for dependency_key in dependency_sections:
      ...
      while index < len(dependencies):
        ...
        if dependency_build_file == target_build_file:
          ...
          raise GypError(((f'Found wildcard in {dependency_key} of ' + target)
  ...
  def FlattenToList(self):
    ...
    while in_degree_zeros:
      ...
      for node_dependent in node.dependents:
        is_in_degree_zero = all(
            node_dependent_dependency.ref in flat_list
...
def MergeDicts(to, fro, to_file, fro_file):
  ...
  for k, v in fro.iteritems():
    ...
    if k in to:
      ...
      if bad_merge:
        raise TypeError(
            ((((f'Attempt to merge dict value of type {v.__class__.__name__}' +
...
def ProcessListFiltersInDict(name, the_dict):
  ...
  for key, value in the_dict.iteritems():
    ...
    if type(value) is not list:
      raise ValueError(
          (f'{name} key {key} must be list, not ' + value.__class__.__name__))
    ...
    if type(the_dict[list_key]) is not list:
      ...
      raise ValueError(((((f'{name} key {list_key}' + ' must be list, not ') +
  ...
  for list_key in lists:
    ...
    if regex_key in the_dict:
      for regex_item in the_dict[regex_key]:
        ...
        if action == 'exclude':
          ...
          raise ValueError(
              ((f'Unrecognized action {action} in {name}' + ' key ') +
               regex_key))
    ...
    # to be created.
    excluded_key = f'{list_key}_excluded'
    if excluded_key in the_dict:
      raise GypError(
          ((f'{name} key {excluded_key}' + ' must not be present prior '
...
def ValidateRulesInTarget(target, target_dict, extra_sources_for_rules):
  ...
  for rule in rules:
    ...
    if 'rule_sources' in rule:
      raise GypError(
          f'rule_sources must not exist in input, target {target} rule {rule_name}'
      )
...
def ValidateActionsInTarget(target, target_dict, build_file):
  ...
  for action in actions:
    ...
    if inputs is None:
      raise GypError(f'Action in target {target_name} has no inputs.')
    action_command = action.get('action')
    if action_command and not action_command[0]:
      raise GypError(f"Empty action as command in target {target_name}.")
...
def PruneUnwantedTargets(targets, flat_list, dependency_nodes, root_targets,
  ...
  for target in root_targets:
    target = target.strip()
    if qualified_targets := gyp.common.FindQualifiedTargets(target, flat_list):
      qualified_root_targets.extend(qualified_targets)

    else:
      raise GypError(f"Could not find target {target}")
  wanted_targets = {}
